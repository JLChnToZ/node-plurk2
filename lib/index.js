  ...
  _createClass(PlurkClient, [{
    ...
    value: function request(api, parameters) {
      ...
      if (parameters) for (var key in parameters) {
        ...
        switch (typeof value === 'undefined' ? 'undefined' : _typeof(value)) {
          case 'undefined':case 'function':
            break;
          case 'object':
            if (value instanceof Date) form[key] = value.toISOString();else form[key] = JSON.stringify(value);
            break;
          default:
            form[key] = value;
            break;
      ...
      return (0, _requestPromise2.default)({
        ...
        method: 'POST', form: form, json: true,
    ...
    value: function startComet() {
      ...
      this.request('Realtime/getUserChannel').then(function (data) {
        ...
        if (_this4.cometStarted) pollComet(_this4);
      }).catch(function (err) {
...
function pollComet(client) {
  ...
  client[pollCometRequest] = (0, _requestPromise2.default)({
    ...
    response = JSON.parse(response.substring(response.indexOf('{'), response.lastIndexOf('}') + 1));
...
function setOAuthParams(client, val) {
  val = _querystring2.default.parse(val);
